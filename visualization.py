...
# Define connections based on the extended number of landmarks (22)
# Ensure this matches the number of landmarks in the data being visualized
connections_extended = config.SKELETON_CONNECTIONS + [(0, config.NUM_LANDMARKS_MEDIAPIPE)] # Add connection to the 22nd point (index 21)
...
def setup_skeleton_plot(ax):
    ...
    # Make panes transparent or semi-transparent
    ax.xaxis.pane.fill = False
    ax.yaxis.pane.fill = False
    ax.zaxis.pane.fill = True # Keep Z pane for reference
    ax.xaxis.pane.set_edgecolor('w')
    ax.yaxis.pane.set_edgecolor('w')
    ax.zaxis.pane.set_edgecolor('w')
    ax.zaxis.pane.set_facecolor((0.95, 0.95, 0.95, 0.6)) # Semi-transparent grey
...
def plot_hand_skeleton(ax, hand_keypoints, color='r', linewidth=1.5, marker='o', markersize=3):
    ...
    if valid_points.shape[0] > 0:
        ax.scatter(valid_points[:, 0], valid_points[:, 1], valid_points[:, 2],
                   marker=marker, s=markersize**2, c=color, depthshade=True) # s is area

    # Plot connections
    for start_idx, end_idx in connections_extended:
        # Ensure indices are within the bounds of the actual data
...
def visualize_frame(keypoints_frame_data, frame_index, title_suffix=""):
    ...
    """
    fig = plt.figure(figsize=(10, 7)) # Adjusted size slightly
    ax = fig.add_subplot(111, projection='3d')
    ax.clear() # Ensure clean axes for each frame
    setup_skeleton_plot(ax)

    num_hands = keypoints_frame_data.shape[0]
    colors = ['r', 'b'] # Colors for different hands
